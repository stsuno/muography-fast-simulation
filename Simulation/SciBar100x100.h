#ifndef SCIBAR100X100_H
#define SCIBAR100X100_H

#include "Simulation/CompositeShape.h"
#include "Simulation/RectangularPrism.h"

class SciBar100x100 : public CompositeShape {
  public:
    SciBar100x100(double cx, double cy, double cz);
    virtual ~SciBar100x100() = default;

    static constexpr int    s_nlayers            = 4;
    static constexpr int    s_nchannel_per_layer = 100;
...
inline SciBar100x100::SciBar100x100(double cx, double cy, double cz) {
...
inline void SciBar100x100::MoveXYZ(double dx, double dy, double dz) {
...
inline void SciBar100x100::RotatePhi(double phi) {
  for (int i=0; i<s_total_channel; i++) {
    std::vector<TVector3> vtx = m_scintillator[i]->GetVertices();
    for (size_t j=0; j<vtx.size(); j++) {
      TVector3 vertex = vtx[j];
...
inline void SciBar100x100::RotateTheta(double theta) {
  for (int i=0; i<s_total_channel; i++) {
    std::vector<TVector3> vtx = m_scintillator[i]->GetVertices();
    for (size_t j=0; j<vtx.size(); j++) {
      TVector3 vertex = vtx[j];
...
inline std::vector<int> SciBar100x100::IsHitList(const Muon& muon) {
...
inline TVector3 SciBar100x100::GetGlobalPositionError(int id) const {
  int layer = GetLayerIndex(id)/2;
  double errx = s_channel_width;
  double erry = s_channel_width*s_nchannel_per_layer;
  if (id>99+200*layer && id<200*(layer+1)) {
