...
int ReconstructionFiberDetector() {
  ...
  std::string rootfile = "fiberdetector.root";
...
//  TVector3 detector_position(0.0,0.0,0.0);
  ...
  ClusterMaker<SciFiberCylinder*> cluster_maker(detector);
  cluster_maker.SetClusterModel(ClusterMaker<SciFiberCylinder*>::GlobalAndTime);

  TrackFinder track_finder(6, 1000000.0);
  ...
  while (std::getline(input_dat, line)) {
    ...
    int hit_id;
    double hit_time;
    std::vector<std::pair<int,double>> hit_ids;
    while (data >> hit_id >> hit_time) { hit_ids.push_back(std::make_pair(hit_id,hit_time)); }
...
int main(int argc, char** argv) {
  return ReconstructionFiberDetector();
}
#endif
